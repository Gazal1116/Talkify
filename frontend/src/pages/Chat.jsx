...
function Chat() {
  ...
  ws.current = new WebSocket("wss://talkify-oo6c.onrender.com");

    ws.current.onopen = () => {
      console.log("✓ WebSocket connected");

      // If sessionId is "existing", don't send it - let backend use the existing session
      const joinData = {
        type: "join",
        room: roomCode,
        username,
      ...
      // Only include sessionId if it's not "existing" (i.e., if it's a new room from Create page)
  ...
  const sendMessage = () => {
    ...
    // Only include sessionId if it's not "existing"
    if (sessionId !== "existing") {
